...
unsigned short simMode = SPATT;
unsigned short swapBytes = 0;
char *simFileName;
char simFile[256];
...
int cioctl(int fd, int request, void *args) {
     ...
     switch (request) {
          case RESET     : if ( (retVal = cioctl(0, RST_PCI, NULL)) == NO_ERROR ) {
                                retVal = cioctl(0, RST_RCO, NULL);
                           } else {
				printf("ERRROR EN RESET !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
			   }
                           break;
...
int copen(char *device, unsigned int mask) {
     ...
     if ( strcmp(device, "SIMULATED_DTC") == 0 ) {
         printf("!!!!!!!!!!!!!!! RUNNING IN SIMULATED MODE !!!!!!!!!!!!!!!!!\n");
         Simulate = 1;
     } else {
...
int sfgread(int id, void *buffer, int size) {
     ...
     if ( buffer == NULL ) {
         printf("**** sfgread: Gota a NULL buffer.\n");
         return -1;
     }
     sprintf((char *)buffer, "%-24s", "** SDSU HEADER INFO **");

     switch (simMode) {
        case SPATT: for (i = 0; i < size - 24; ++i) {
                        *p++ = counter;
	...
	case SFILE: tbuf = (void *)malloc(size * sizeof(unsigned short));
                    fp = open(simFile, O_RDONLY, 0000);
                    if ( read(fp, tbuf, size * sizeof(unsigned short)) == -1 )
                        memset(tbuf, 128, size * sizeof(unsigned short));
                    close(fp);

                    if ( swapBytes ) {
                        p = (unsigned short *)buffer + 12;
                        swab(tbuf, (void *)p, size * sizeof(unsigned short));
                    }
    
                    free(tbuf);
                    break;

        default:    printf("**** sfgread: bad simMode %d\n", simMode);
                    return -1;
     }
...
int fgopen(char *device, unsigned int mask) {
     ...
     if ( strcmp(device, "SIMULATED_FGR") == 0 ) {
         Simulate = 1;
         simMode = SPATT;
         printf("RUNNING SIMULATED MODE PATTERN\n");
         /* If simMode is file, set the image files directory */
     } else 
         if ( strcmp(device, "SDSU_FGR") == 0 ) {
             Simulate = 0;
             simMode = 0;
             ...
             Simulate = 1;
             simMode = SFILE;
             sprintf(simFile, "%s", device);
             printf("RUNNING SIMULATED MODE WITH FILE\n");
         }
